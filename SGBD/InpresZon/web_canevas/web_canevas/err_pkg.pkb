create or replace package body err_pkg is
	g_target pls_integer     := c_table + c_file;
	g_file   varchar2 (2000) := 'log.txt';
	...
	*/
	...
	begin
		if bitand (g_target, c_table) = c_table then
			insert into log (id, message, errcode) values (seq_log.nextval, l_sqlerrm, l_sqlcode);
			commit;
		end if;
		...
		if bitand (g_target, c_file) = c_file then
			...
			begin
				...
				utl_file.put_line (fid, l_sqlerrm);
	...
	function find_constraint_name return my_constraints.name%TYPE as
		res     owa_text.vc_arr;
		found   boolean;
	begin
	...
	return my_constraints.message%TYPE as
 ...
 **************************************************************/
	...
	begin
		if not condition or condition is null then
			log(errmsg => 'Assertion Failure: ' || message); -- sqlcode equals 0 !
