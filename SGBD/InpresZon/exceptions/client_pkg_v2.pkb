 ...
 **************************************************************/
      ...
      exception
         when others then
            rollback;
            raise_application_error(-20002, 'échec lors de l''insertion: ' || SQLERRM);
      end;
/**************************************************************
 * MAIN CODE
 **************************************************************/
   ...
   exception
      when validation_ex then
         err_pkg.report_and_go('client_pkg.ajout_client', errnums_pkg.client_ajout_const, 'Problème de validation', true);
      when insertion_ex then
         err_pkg.report_and_go('client_pkg.ajout_client', errnums_pkg.client_ajout_const, 'Problème d''insertion', true);
      when others then
